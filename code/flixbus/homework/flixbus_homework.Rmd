...
```{r echo=FALSE, message=FALSE, warning=FALSE}
pointsS[, list(mean_tip_pct=mean(tip_pct), prevalence=nrow(.SD)/nrow(points)), by=group]
```
...
Filtering out regions that had less than ten trips overall, here is the percentage of trips from each region that had a zero tip:
...
Tipping behavior was shown to be driven by several factors: a categorical decision to pay zero tip, or to pay with a card preset amount of 20%, 25%, or 30%, or to pay another amount.  A fully specified model of tipping behavior could be consructed to include variables for both the categorial decision and the continuous tip amount decision.  From here you could estimate expected tip.

Tip amount varies strongly with geography.  Regions near the city centre in Manhattan and Brooklyn are more likely to pay with a card preset, less likely to leave a zero tip, and leave a higher average tip when they pay another amount.  Regions towards the edge of the city are more likely to leave a zero tip, less likely to pay with a card preset, and leave a lower average tip otherwise.  An exception to this are the two airport zones, JFK and La Guardia, which exhibit high average tips comparable to the city centre.
