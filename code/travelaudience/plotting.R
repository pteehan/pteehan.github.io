...
plotAirports <- function(codes=c("FRA", "LHR")) {
  ...
  world <- ggplot(world, aes(x = long, y = lat)) + 
                 ...
                 fill = "#f9f9f9", size = 0.2) +
    coord_quickmap(xlim = c(lonmin, lonmax),ylim = c(latmin, latmax))
  world + geom_point(data=airports, aes(x=lng, y=lat)) + 
    geom_text(data=airports, aes(label=code, x=lng, y=lat), nudge_y=2, size=3)
}